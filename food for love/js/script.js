...
function loadedSucc()
{
	var oParent=document.getElementById('photo-gallery');
	...
	var aDiv=oParent.getElementsByTagName('photo-gallery');
	...
	for(var j=0;j<ROW;j++)
	{
		for(var i=0;i<COL;i++,k++)
		{
			...
			var oDiv=document.createElement('photo-gallery');
	...
	oPrev.onclick=oNext.onclick=function ()
	{
		...
		var timer=setInterval(function (){
			...
			aDiv[item].getElementsByTagName('span')[0].style.background='url(images/'+(iNow+1)+'.jpg) '+-aDiv[item].wc_col*dw+'px '+-aDiv[item].wc_row*dh+'px';
